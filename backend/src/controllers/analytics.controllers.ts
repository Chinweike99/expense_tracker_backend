...
export const getDashboardStats = async(req: Request, res: Response) => {
    try {
        ...
        const { startDate, endDate, accounts, categories, type} = analyticsQuerySchema.parse(req.query);
    ...
    const accountsData = await Account.find({user: userId, isActive: true}).select("name balance currency type");
    ...
    const prevYearFilter = { 
      ...
      Transaction.aggregate([
        ...
        const getTotal = (result: any[]) => result.length > 0 ? result[0].total : 0;

        // Calculate trends
    const monthlyExpenseTrend = prevMonthExpenses.length > 0 
    ? (getTotal(currentMonthExpenses) - getTotal(prevMonthExpenses)) / getTotal(prevMonthExpenses) * 100 : 0;

    const yearlyExpenseTrend = prevYearExpenses.length > 0 
      ? (getTotal(currentYearExpenses) - getTotal(prevYearExpenses)) / getTotal(prevYearExpenses) * 100 
      : 0;
     ...
     const categoryDetails = await Category.find({
      ...
      const enrichedCategorySpending = categorySpending.map((cat: any) => {
        const detail = categoryDetails.find((d) => (d as any)._id.equals(cat._id));
        return {
          ...cat,
...
export const getSpendingTrends = async(req: Request, res: Response) => {
    try {
        ...
        const formattedTrends = spendingTrends.map((trend) => {
            let label;
      if (period === 'day') {
        label = `${trend._id.year}-${trend._id.month.toString().padStart(2, '0')}-${trend._id.day.toString().padStart(2, '0')}`;
      } else if (period === 'week') {
        label = `Week ${trend._id.week}, ${trend._id.year}`;
      } else if (period === 'month') {
        label = `${new Date(trend._id.year, trend._id.month - 1).toLocaleString('default', { month: 'short' })} ${trend._id.year}`;
      } else if (period === 'year') {
  ...
  export const getCategoryComparison = async (req: Request, res: Response) => {
    try {
      ...
      const { startDate, endDate, accounts } = analyticsQuerySchema.parse(req.query);
      ...
      if (compareWith === 'previousPeriod') {
        if (startDate && endDate) {
          const duration = moment(endDate).diff(moment(startDate), 'milliseconds');
      ...
      const result = allCategoryIds.map((categoryId) => {
        const current = currentCategories.find((c) => c._id?.equals(categoryId)) || { total: 0, count: 0 };
        const comparison = comparisonCategories.find((c) => c._id?.equals(categoryId)) || { total: 0, count: 0 };
        const category = categoryDetails.find((d) => (d._id as any).equals(categoryId));
  
        // Calculate percentage change
        const percentageChange = comparison.total !== 0 
...
export const getExpenseHeatMap = async(req: Request, res: Response) => {
    try {
        const userId = req.user.id;
        const {startDate, endDate, accounts, categories} = analyticsQuerySchema.parse(req.query);
...
export const exportTransactions = async(req: Request, res: Response) => {
    try {
        ...
        const transactions = await Transaction.find(filter).populate('account', 'name currency').populate('category', 'name').sort({date: -1});

        if (format === 'csv'){
            // Convert to csv
            const header = [
                'Date', 'Type', 'Amount', 'Currency', 'Account', 'Category', 'Description', 'Tags', 'Notes'
            ].join(',');
