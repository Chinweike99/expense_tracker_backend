...
export const createAccount = async (
  ...
  try {
    ...
    const { name, type, balance, currency } = createAccountSchema.parse(
      req.body
    );
    ...
    if (!currencyExist) {
      res
        .status(400)
        .json({ message: "Currency not found. Please add it first" });
      return;
    }
...
export const addCurrency = async (
  ...
  try {
    const userId = req.user.id;
    const { code, name, symbol, isPrimary } = createCurrencySchema.parse(
      req.body
    );
    ...
    if (!currency.isPrimary) {
      const primaryCurrency = await Currency.findOne({
        user: userId,
        isPrimary: true,
      });
      if (primaryCurrency) {
...
export const setPrimaryCurrency = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
...
export const deleteCurreny = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    if (currency.isPrimary) {
      res
        ...
        .json({
          message:
            "Cannot delete primary currency, choose a new Primary currency as to delete",
        });
      return;
    }
    ...
    if (accountsUsingCurrency > 0) {
      res.status(400).json({
        message:
          "Cannot delete currency as it is being used by one or more accounts",
      });
      return;
    }
